...
void main(){
    vec3 albedo = vec3(0.123,0.34,0.8);
    float metallic = 0;
    float roughness = 1;
    ...
    //vec3 normal = normalize(vertex_basis);
    ...
    vec3 ambient = ambient_color * albedo * 1.0;
    vec3 color = ambient + lighted;

    float alpha = min((1-dot(normal, view_direction))*0.45,1.0);
